...
public:
 ...
 public:
  ...
  void load_lumi_var(const LumiData &lumi) {
    ...
    lumi_instlumi = (lumi.run<190389) ? lumi.instlumi : (runls_instlumi.count(runls) ? (good_instlumi ? runls_instlumi[runls] / 23.3104 : NOVAL_F) : NOVAL_F); // (/nb)
    instlumi_p1 = (lumi_instlumi==NOVAL_F) ? NOVAL_I : 1+lumi_instlumi/1.5;
...
#if VERSION1 > 30
    traj_instlumi = (e.run==1) ? 0.221995*(e.pileup) : e.instlumi_ext/1000; // (nb-1s-1)
