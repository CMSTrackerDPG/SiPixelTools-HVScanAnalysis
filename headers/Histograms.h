...
/* 	  if (i==3&&j==0) fit_treshold_(h2d_3p_[i][j][2]); */
  ...
  void add_(TFile &f, TH1D* h) { h->Add((TH1D*)f.Get(h->GetName())); }
  void add_(TFile &f, TH2D* h) { h->Add((TH2D*)f.Get(h->GetName())); }
  void add_(TFile &f, TH3D* h) { h->Add((TH3D*)f.Get(h->GetName())); }
  void load_(TFile &f, TH1D* h) {
    TH1D *h_temp = (TH1D*)f.Get(h->GetName());
    if (h_temp!=0) { delete h; h = h_temp; h->SetDirectory(0); }
  }
  void load_(TFile &f, TH2D* h) {
    TH2D *h_temp = (TH2D*)f.Get(h->GetName());
    if (h_temp!=0) { delete h; h = h_temp; h->SetDirectory(0); }
  }
  void load_(TFile &f, TH3D* h) {
    TH3D *h_temp = (TH3D*)f.Get(h->GetName());
    if (h_temp!=0) { delete h; h = h_temp; h->SetDirectory(0); }
  }
 ...
 public:
  ...
  // Load
  void load(TFile &f, bool add=false) {
    std::cout<<"Loading histo: "<<histoname_<<std::endl;
    if (npostfix_==1) {
      for (size_t i=0; i<h1d_1p_.size(); i++) {
	if (add) add_(f, h1d_1p_[i]);
	else  load_(f, h1d_1p_[i]);
      }
      for (size_t i=0; i<h2d_1p_.size(); i++) {
	if (add) add_(f, h2d_1p_[i]);
	else  load_(f, h2d_1p_[i]);
      }
      for (size_t i=0; i<h3d_1p_.size(); i++) {
	if (add) add_(f, h3d_1p_[i]);
	else  load_(f, h3d_1p_[i]);
      }
    } else if (npostfix_==2) {
      for (size_t i=0; i<h1d_2p_.size(); i++) for (size_t j=0; j<h1d_2p_[i].size(); j++) {
	if (add) add_(f, h1d_2p_[i][j]);
	else  load_(f, h1d_2p_[i][j]);
      }
      for (size_t i=0; i<h2d_2p_.size(); i++) for (size_t j=0; j<h2d_2p_[i].size(); j++) {
	if (add) add_(f, h2d_2p_[i][j]);
	else  load_(f, h2d_2p_[i][j]);
      } 
      for (size_t i=0; i<h3d_2p_.size(); i++) for (size_t j=0; j<h3d_2p_[i].size(); j++) {
	if (add) add_(f, h3d_2p_[i][j]);
	else  load_(f, h3d_2p_[i][j]);
      }
    } else if (npostfix_==3) {
      if (def_==1) {
	if (add) add_(f, h1d_1p_[0]);
	else  load_(f, h1d_1p_[0]);
      }
      for (size_t i=0; i<h1d_3p_.size(); i++) for (size_t j=0; j<h1d_3p_[i].size(); j++) {
	for (size_t k=0; k<h1d_3p_[i][j].size(); k++) {
	  if (add) add_(f, h1d_3p_[i][j][k]);
	  else  load_(f, h1d_3p_[i][j][k]);
	}
      }
      for (size_t i=0; i<h2d_3p_.size(); i++) for (size_t j=0; j<h2d_3p_[i].size(); j++) for (size_t k=0; k<h2d_3p_[i][j].size(); k++) {
	if (add) add_(f, h2d_3p_[i][j][k]);
	else  load_(f, h2d_3p_[i][j][k]);
      } 
      for (size_t i=0; i<h3d_3p_.size(); i++) for (size_t j=0; j<h3d_3p_[i].size(); j++) for (size_t k=0; k<h3d_3p_[i][j].size(); k++) {
	if (add) add_(f, h3d_3p_[i][j][k]);
	else  load_(f, h3d_3p_[i][j][k]);
      }
    } else if (npostfix_==4) {
      for (size_t i=0; i<h1d_4p_.size(); i++) for (size_t j=0; j<h1d_4p_[i].size(); j++) 
	for (size_t k=0; k<h1d_4p_[i][j].size(); k++) for (size_t l=0; l<h1d_4p_[i][j][k].size(); l++) {
	  if (add) add_(f, h1d_4p_[i][j][k][l]);
	  else  load_(f, h1d_4p_[i][j][k][l]);
	}
	...
	for (size_t k=0; k<h2d_4p_[i][j].size(); k++) for (size_t l=0; l<h2d_4p_[i][j][k].size(); l++) {
	  if (add) add_(f, h2d_4p_[i][j][k][l]);
	  else  load_(f, h2d_4p_[i][j][k][l]);
	}
	...
	for (size_t k=0; k<h3d_4p_[i][j].size(); k++) for (size_t l=0; l<h3d_4p_[i][j][k].size(); l++) {
	  if (add) add_(f, h3d_4p_[i][j][k][l]);
	  else  load_(f, h3d_4p_[i][j][k][l]);
	}
    }
  }
  ...
  void partial_load(TFile &f, int i, int refill=0) {
	...
	if (refill!=1) {
	  ...
	  h->SetDirectory(NULL);
	  for (int a=1; a<=h->GetNbinsX(); a++) {
	    int binx = h1d_1p_[i]->FindBin(h->GetXaxis()->GetBinCenter(a));
	...
	if (refill!=1) {
	  ...
	  h->SetDirectory(NULL);
	  for (int a=1; a<=h->GetNbinsX(); a++) {
	    int binx = h1d_2p_[i][j]->FindBin(h->GetXaxis()->GetBinCenter(a));
	...
	for (size_t k=0; k<h1d_3p_[i][j].size(); k++) {
	  ...
	  if (refill!=1) {
	    ...
	    h->SetDirectory(NULL);
	    for (int a=1; a<=h->GetNbinsX(); a++) {
	      int binx = h1d_3p_[i][j][k]->FindBin(h->GetXaxis()->GetBinCenter(a));
	...
	for (size_t k=0; k<h1d_4p_[i][j].size(); k++) {
	  for (size_t l=0; l<h1d_4p_[i][j][k].size(); l++) {
		...
		h->SetDirectory(NULL);
		for (int a=1; a<=h->GetNbinsX(); a++) {
		  int binx = h1d_4p_[i][j][k][l]->FindBin(h->GetXaxis()->GetBinCenter(a));
...
/* 	  if (aoi>0.2&&aoi<0.27) { */
  ...
  TGraphAsymmErrors* asym_(TH1D* eff, TH1D* den) {
    ...
    int n = 0;
    for (int i=0; i<eff->GetNbinsX(); ++i) if (eff->GetBinContent(i+1)>0) n++;
