...
class AllHistos {
 ...
 private:
  ...
  void evthistos_fill_(Variables &v, const EventData &e) {
	...
	dynamic_ineff ->h1d(3, lay-1,  0, 2)->Fill(0.221995*(e.pileup-1), e.npix[lay]);
	dynamic_ineff ->h1d(3, lay-1,  0, 3)->Fill(0.221995*(e.pileup-1), e.nclu[lay]);
	dynamic_ineff ->h1d(3, lay-1,  0, 4)->Fill(0.221995*(e.pileup-1));
	...
	dynamic_ineff ->h1d(4, lay-1,  0, 4)->Fill(e.pileup-1);
      ...
      for (int lay=1; lay<=3; lay++) {
	dynamic_ineff ->h1d(3, lay-1,  1, 2)->Fill(0.221995*(e.pileup-1), e.npix[lay]);
	dynamic_ineff ->h1d(3, lay-1,  1, 3)->Fill(0.221995*(e.pileup-1), e.nclu[lay]);
	dynamic_ineff ->h1d(3, lay-1,  1, 4)->Fill(0.221995*(e.pileup-1));
	dynamic_ineff ->h1d(4, lay-1,  0, 2)->Fill(e.pileup-1, e.npix[lay]);
	dynamic_ineff ->h1d(4, lay-1,  0, 3)->Fill(e.pileup-1, e.nclu[lay]);
	dynamic_ineff ->h1d(4, lay-1,  0, 4)->Fill(e.pileup-1);
      }
  ...
  void non_efficiency_fill_(Variables &v, const TrajMeasurement &t, const EventData &e) {
	...
	resol->h2d(isMC, t.mod_on.layer-1, 2, 0)->Fill(v.traj_instlumi, t.res_dx);
	resol->h2d(isMC, t.mod_on.layer-1, 2, 1)->Fill(v.traj_instlumi, t.res_dz);
  ...
  void efficiency_fill_(Variables &v, const TrajMeasurement &t, const EventData &e) {
    ...
    if (v.zb&&!t.mod_on.det&&t.clu.size) {
      double clu_charge = 0; // if not in SPLIT2 mode
      for (int i=0; i<t.clu.size; i++) clu_charge+=t.clu.adc[i];
      double clu_normch = t.norm_charge;
      if (e.run!=1) { // Data
	dynamic_ineff ->h2d(0, t.mod_on.layer-1,  0, 0)->Fill(t.mod_on.ladder, clu_charge);
	dynamic_ineff ->h2d(1, t.mod_on.layer-1,  0, 0)->Fill(t.mod_on.module, clu_charge);
	dynamic_ineff ->h2d(2, t.mod_on.layer-1,  0, 0)->Fill(v.traj_instlumi, clu_charge);
	dynamic_ineff ->h2d(0, t.mod_on.layer-1,  0, 1)->Fill(t.mod_on.ladder, t.norm_charge);
	dynamic_ineff ->h2d(1, t.mod_on.layer-1,  0, 1)->Fill(t.mod_on.module, t.norm_charge);
	dynamic_ineff ->h2d(2, t.mod_on.layer-1,  0, 1)->Fill(v.traj_instlumi, t.norm_charge);
	for (int i=0; i<t.clu.size; i++) {
	  dynamic_ineff ->h2d(0, t.mod_on.layer-1,  0, 2)->Fill(t.mod_on.ladder, t.clu.adc[i]);
	  dynamic_ineff ->h2d(1, t.mod_on.layer-1,  0, 2)->Fill(t.mod_on.module, t.clu.adc[i]);
	  dynamic_ineff ->h2d(2, t.mod_on.layer-1,  0, 2)->Fill(v.traj_instlumi, t.clu.adc[i]);
	}
      } else { // MC
	// Apply gain correction to MC cluster charges
	clu_charge *= gain_factor_ring_[0][t.mod_on.layer-1][abs(t.mod_on.module)-1];
	clu_normch *= gain_factor_ring_[1][t.mod_on.layer-1][abs(t.mod_on.module)-1];
	//std::cout<<"layer"<<t.mod_on.layer<<" ring"<<abs(t.mod_on.module)<<" factor="<<gain_factor_ring_[1][t.mod_on.layer-1][abs(t.mod_on.module)-1]
	//	 <<" norm="<<t.norm_charge<<" corr="<<clu_normch<<std::endl;
	//std::cout<<dynamic_ineff ->h2d(1, t.mod_on.layer-1,  1, 1)->GetName()<<std::endl;
#if VERSION2 >= 35
	//dynamic_ineff ->h2d(0, t.mod_on.layer-1,  1, 0)->Fill(t.mod_on.ladder, clu_charge, e.weight);
	//dynamic_ineff ->h2d(1, t.mod_on.layer-1,  1, 0)->Fill(t.mod_on.module, clu_charge, e.weight);
	//dynamic_ineff ->h2d(2, t.mod_on.layer-1,  1, 0)->Fill(v.traj_instlumi, clu_charge, e.weight);
	//dynamic_ineff ->h2d(0, t.mod_on.layer-1,  1, 1)->Fill(t.mod_on.ladder, clu_normch, e.weight);
	//dynamic_ineff ->h2d(1, t.mod_on.layer-1,  1, 1)->Fill(t.mod_on.module, clu_normch, e.weight);
	//dynamic_ineff ->h2d(2, t.mod_on.layer-1,  1, 1)->Fill(v.traj_instlumi, clu_normch, e.weight);
	dynamic_ineff ->h2d(0, t.mod_on.layer-1,  1, 0)->Fill(t.mod_on.ladder, clu_charge, 1);
	dynamic_ineff ->h2d(1, t.mod_on.layer-1,  1, 0)->Fill(t.mod_on.module, clu_charge, 1);
	dynamic_ineff ->h2d(2, t.mod_on.layer-1,  1, 0)->Fill(v.traj_instlumi, clu_charge, 1);
	dynamic_ineff ->h2d(0, t.mod_on.layer-1,  1, 1)->Fill(t.mod_on.ladder, clu_normch, 1);
	dynamic_ineff ->h2d(1, t.mod_on.layer-1,  1, 1)->Fill(t.mod_on.module, clu_normch, 1);
	dynamic_ineff ->h2d(2, t.mod_on.layer-1,  1, 1)->Fill(v.traj_instlumi, clu_normch, 1);
	for (int i=0; i<t.clu.size; i++) {
	  dynamic_ineff ->h2d(0, t.mod_on.layer-1,  1, 2)->Fill(t.mod_on.ladder, t.clu.adc[i], e.weight);
	  dynamic_ineff ->h2d(1, t.mod_on.layer-1,  1, 2)->Fill(t.mod_on.module, t.clu.adc[i], e.weight);
	  dynamic_ineff ->h2d(2, t.mod_on.layer-1,  1, 2)->Fill(v.traj_instlumi, t.clu.adc[i], e.weight);
	}
#else
	dynamic_ineff ->h2d(0, t.mod_on.layer-1,  1, 0)->Fill(t.mod_on.ladder, clu_charge);
	dynamic_ineff ->h2d(1, t.mod_on.layer-1,  1, 0)->Fill(t.mod_on.module, clu_charge);
	dynamic_ineff ->h2d(2, t.mod_on.layer-1,  1, 0)->Fill(v.traj_instlumi, clu_charge);
	dynamic_ineff ->h2d(0, t.mod_on.layer-1,  1, 1)->Fill(t.mod_on.ladder, clu_normch);
	dynamic_ineff ->h2d(1, t.mod_on.layer-1,  1, 1)->Fill(t.mod_on.module, clu_normch);
	dynamic_ineff ->h2d(2, t.mod_on.layer-1,  1, 1)->Fill(v.traj_instlumi, clu_normch);
	for (int i=0; i<t.clu.size; i++) {
	  dynamic_ineff ->h2d(0, t.mod_on.layer-1,  1, 2)->Fill(t.mod_on.ladder, t.clu.adc[i]);
	  dynamic_ineff ->h2d(1, t.mod_on.layer-1,  1, 2)->Fill(t.mod_on.module, t.clu.adc[i]);
	  dynamic_ineff ->h2d(2, t.mod_on.layer-1,  1, 2)->Fill(v.traj_instlumi, t.clu.adc[i]);
	}
#endif
      }
    }
	...
	if (e.run==201278&&!(e.ls<62||(e.ls>163&&e.ls<166)||(e.ls>229&&e.ls<232)||(e.ls>256&&e.ls<259)||(e.ls>316&&e.ls<318)||(e.ls>595&&e.ls<598)||(e.ls>938&&e.ls<942)||(e.ls>974&&e.ls<976)||(e.ls>1160&&e.ls<1163)||(e.ls>1304&&e.ls<1306)||(e.ls>1793&&e.ls<1796)||(e.ls>1802&&e.ls<1805)||(e.ls>1906&&e.ls<1909)||(e.ls>1929&&e.ls<1932)||e.ls>2174)) {
	  if (!t.mod_on.det) {
	    ...
	    dynamic_ineff ->h1d(4, t.mod_on.layer-1,  0, t.missing)->Fill(v.traj_instlumi/0.221995);
	    dynamic_ineff ->h2d(0, t.mod_on.layer-1,  0, 0)->Fill(t.mod_on.ladder, t.norm_charge);
	    dynamic_ineff ->h2d(1, t.mod_on.layer-1,  0, 0)->Fill(t.mod_on.module, t.norm_charge);
	    dynamic_ineff ->h2d(2, t.mod_on.layer-1,  0, 0)->Fill(v.traj_instlumi, t.norm_charge);
	  }
	...
	if (!t.mod_on.det) {
	  ...
	  dynamic_ineff ->h1d(4, t.mod_on.layer-1,  1, t.missing)->Fill(v.traj_instlumi/0.221995, e.weight);
	  //dynamic_ineff ->h1d(2, t.mod_on.layer-1,  1, t.missing)->Fill(v.traj_l1rate, e.weight);
	  dynamic_ineff ->h2d(0, t.mod_on.layer-1,  1, 0)->Fill(t.mod_on.ladder, t.norm_charge, e.weight);
	  dynamic_ineff ->h2d(1, t.mod_on.layer-1,  1, 0)->Fill(t.mod_on.module, t.norm_charge, e.weight);
	  dynamic_ineff ->h2d(2, t.mod_on.layer-1,  1, 0)->Fill(v.traj_instlumi, t.norm_charge, e.weight);
#else
	  ...
	  dynamic_ineff ->h1d(4, t.mod_on.layer-1,  1, t.missing)->Fill(v.traj_instlumi/0.221995);
	  //dynamic_ineff ->h1d(2, t.mod_on.layer-1,  1, t.missing)->Fill(v.traj_l1rate);
	  dynamic_ineff ->h2d(0, t.mod_on.layer-1,  1, 0)->Fill(t.mod_on.ladder, t.norm_charge);
	  dynamic_ineff ->h2d(1, t.mod_on.layer-1,  1, 0)->Fill(t.mod_on.module, t.norm_charge);
	  dynamic_ineff ->h2d(2, t.mod_on.layer-1,  1, 0)->Fill(v.traj_instlumi, t.norm_charge);
#endif
...
// 	    YMax = f->GetMaximum();
  ...
  TH1D* getslice(TH2D* h2d, int binx, std::string pf="") {
    std::stringstream ss;
    ss<<h2d->GetName();
    if (pf.size()>0) ss<<pf;
    ss<<"_bin"<<binx;
    const char* name = ss.str().c_str();
    TH1D* h1d = new TH1D(name, name, h2d->GetNbinsY(), h2d->GetYaxis()->GetXmin(), h2d->GetYaxis()->GetXmax());
    for (int biny=1; biny<=h2d->GetNbinsY(); biny++) {
      double cont = h2d->GetBinContent(binx,biny);
      if (cont>0) h1d->SetBinContent(biny,cont);
    }
    return h1d;
  }

  // Calculate gain correction factors
  double gain_factor_ring_[2][3][4];
  TF1* fit_landau_(TH1D* h, bool norm, double low=0, double high=100) {
    if (low<0) low=0;
    if (norm&&high>100) high=100;
    if (!norm&&high>200) high=200;
    //h->Draw();
    TF1 *f = new TF1("fit","landau", low, high);
    h->Fit("fit","RQ0");
    //f->Draw("SAME");
    //gPad->Update();
    //if (low==0) cout<<"Fit: "<<f->GetParameter(1)<<" ";
    //else cout<<"Refit: "<<f->GetParameter(1)<<endl;
    //sleep(2);
    return f;
  }
  
  double get_mpv_(TH1D* h, bool norm, bool l1r2) {
    TF1* fit = fit_landau_(h,norm,0,norm?100:200);
    double mpv = fit->GetParameter(1);
    double sigma = fit->GetParameter(2);
    //cout<<sigma<<std::endl;
    // Refit
    delete fit;
    double nsig_min  = norm ? 2 : (sigma>10) ? 2 : 2;
    double nsig_plus = norm ? 4 : (sigma>10) ? 3 : 4;
    fit = fit_landau_(h,norm,mpv-nsig_min*sigma,mpv+nsig_plus*sigma);
    mpv = fit->GetParameter(1);
    sigma = fit->GetParameter(2);
    // Refit once more (except for l1 ring2) which needs wider range
    if (!norm&&!l1r2) {
      delete fit;
      fit = fit_landau_(h,norm,mpv-1.5*sigma,mpv+1.5*sigma);
      mpv = fit->GetParameter(1);
      sigma = fit->GetParameter(2);
    }
    return mpv;
  ...
  double get_gain_corr_(TH1D* data, TH1D* mc, bool norm, bool l1r2) {
    if (data->GetEntries()>0&&mc->GetEntries()>0) {
      // Clone and normalize
      TH1D* data_clone = (TH1D*)data->Clone();
      TH1D* mc_clone = (TH1D*)mc->Clone();
      data_clone->Scale(1/data_clone->Integral());
      mc_clone->Scale(1/mc_clone->Integral());
      //data_clone->SetMarkerStyle(20);
      //data_clone->Draw("P");
      //mc_clone->SetLineColor(2);
      //mc_clone->Draw("SAME");
      //double data_mpv = get_mpv_(data_clone, norm, l1r2);
      //double mc_mpv   = get_mpv_(mc_clone, norm, l1r2);
      //double corr = data_mpv/mc_mpv;
      //std::cout<<data_mpv<<" "<<mc_mpv<<std::endl;
      double corr = get_mpv_(data_clone, norm, l1r2)/get_mpv_(mc_clone, norm, l1r2);
      return corr;
    } else return -9999.9;
  }
...
public:
  ...
  void load_gain_factors(bool load=false) {
    if (load) {
      // Load histograms, and recalculate factors
      TFile f_data("/data/jkarancs/CMSSW/TimingStudy/CMSSW_7_1_0_pre1/src/DPGAnalysis/PixelTimingStudy/test/PixelHistoMaker/Data_201278.root");
      dynamic_ineff->load(f_data,1); f_data.Close();
      TFile f_mc("/data/jkarancs/CMSSW/TimingStudy/CMSSW_7_1_0_pre1/src/DPGAnalysis/PixelTimingStudy/test/PixelHistoMaker/Dcol100.root");
      dynamic_ineff->load(f_mc,1); f_mc.Close();
      TH1D* ring_charge[2][3][4][2];
      const char* dataname[2] = {"_Data", "_MC"};
      const char* layname[3] = {"_l1", "_l2","_l3"};
      const char* ringname[4] = {"_ring1", "_ring2", "_ring3", "_ring4"};
      //std::cout<<"Gain correction factors:"<<std::endl;
      for (int norm=0; norm<=1; norm++) for (int lay=0; lay<3; lay++) for (int ring=0; ring<4; ring++) {
        for (int data=0; data<=1; data++) {
          int nbin = dynamic_ineff->h2d(1, lay, data, norm)->GetNbinsY();
          double low = dynamic_ineff->h2d(1, lay, data, norm)->GetYaxis()->GetXmin(), high = dynamic_ineff->h2d(1, lay, data, norm)->GetYaxis()->GetXmax();
          ring_charge[norm][lay][ring][data] = new TH1D((std::string(norm?"normch":"charge")+layname[lay]+ringname[ring]+dataname[data]).c_str(),
          					      (norm?";Normalized cluster charge (ke);":";Cluster charge (ke);"),nbin,low,high);
          ring_charge[norm][lay][ring][data]->Add(getslice(dynamic_ineff->h2d(1, lay,  data, norm),(4-ring)));
          ring_charge[norm][lay][ring][data]->Add(getslice(dynamic_ineff->h2d(1, lay,  data, norm),(5+ring)));
        }
        bool l1r2 = (lay==0&&ring==1); // The landau fit of l1 ring1 is very bad, so use wider range
        gain_factor_ring_[norm][lay][ring] = get_gain_corr_(ring_charge[norm][lay][ring][0], ring_charge[norm][lay][ring][1], norm, l1r2);
        //if (norm) gain_factor_ring_[norm][lay][ring] *= someonepleasetellmewhyineedthisfactor[lay][ring];
        std::cout<<" Layer"<<lay+1<<" Ring"<<ring+1<<": "<<gain_factor_ring_[norm][lay][ring]<<std::endl;
      }
    } else {
      double factors[2][3][4] = {
	{ 
	  { 0.87311,  0.863504, 0.926368, 0.917721 },
	  { 0.950377, 0.97627,  1.04941,  0.98619  },
	  { 1.01047,  1.03441,  1.06902,  1.0403   }, 
	},
	{ 
	  { 0.865666, 0.871703, 0.879282, 0.907823 },
	  { 0.959228, 0.961938, 0.971218, 0.975718 },
	  { 1.0298,   1.03292,  1.02963,  1.03261  } 
	}
      };
      for (int norm=0; norm<=1; norm++) for (int lay=0; lay<3; lay++) 
	for (int ring=0; ring<4; ring++) gain_factor_ring_[norm][lay][ring] = factors[norm][lay][ring];
    }
  }
