...
public:
 ...
 private:
  ...
  void common_(const EventData& e, const ModuleData& m) {
    ...
    pf_bpix_l1_new  = m.rawid == 303054876||
                      m.rawid == 303063068||
                      m.rawid == 303054864||
                      m.rawid == 303054856||
                      m.rawid == 303063056||
    ...
    bias_voltage = scans::bias_voltage(pf_hv_scan, e.run, e.ls, e.orb, m.det, m.layer, abs(m.disk), pf_bpix_l1_new);
...
public:
  ...
  void load_lumi_var(const LumiData &lumi) {
    ...
    badroc_runnumber = scans::hv_scan_badroc_runnumber(lumi.run);
#ifndef EXPRESS
    if (lumi.run<238474) { 
      // Run I
  ...
  void load_lumi_var_phase1(const EventData &e) {
	...
	lumi_pileup      = fromfile_pileup[runls];
      ...
      // Load new ROC selection
    badroc_runnumber = scans::hv_scan_badroc_runnumber(e.run);
#ifndef EXPRESS
      if (e.run<238474) { 
	// Run I
  ...
  void load_traj_var(const EventData &e, const TrajMeasurement &t) {
    ...
    bool goodmod_RunII_phase1_2018 = 1;
    //goodmod = (e.run==1) ? 1 : e.run<238474 ? goodmod_RunI : e.run<285000 ? goodmod_RunII_phase0 : goodmod_RunII_phase1;
    goodmod = (e.run==1) ? 1 : e.run<238474 ? goodmod_RunI : e.run<285000 ? goodmod_RunII_phase0 : e.run<314649 ? goodmod_RunII_phase1 : goodmod_RunII_phase1_2018;//not sure if needed, for 2018 no bad modules are selected this way
  ...
  bool edge_ly = m.det==0 ? fabs(t.ly)<3.1 :
    ...
    // Try out new hit efficiency cuts for old measurement method
    pt = pt_new;
    //d0 = d0_new;
    //dz = dz_new;
    ...
    //effcut_allmod      =       cut_nvtx && cut_federr && hp && pt && nstrip && d0 && dz && pixhit           && badroc_behind_dcdc && lx_fid && ly_fid && valmis && hitsep && trk_beta_cut;
